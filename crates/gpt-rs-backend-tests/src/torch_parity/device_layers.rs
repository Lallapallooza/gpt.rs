...
pub fn linear_initializes_from_host_and_device_weights<B: PortableBackend + 'static>(
    ...
    timed_gpt(|| {
        ...
        let host_layer =
            Linear::new(Arc::clone(backend), weight.clone(), Some(bias.clone())).unwrap();
        ...
        .unwrap();
        let refs_layer =
            Linear::new(Arc::clone(backend), &weight_device, Some(&bias_device)).unwrap();
...
pub fn embedding_initializes_from_device_tensor<B: PortableBackend + 'static>(backend: &Arc<B>) {
    timed_gpt(|| {
        ...
        let second = Embedding::new(Arc::clone(backend), device_weight.clone()).unwrap();
...
pub fn feed_forward_accepts_mixed_tensor_inputs<B: PortableBackend + 'static>(backend: &Arc<B>) {
    timed_gpt(|| {
        ...
        let w_out_device = DeviceTensor::from_host(Arc::clone(backend), w_out.clone()).unwrap();
        let b_out_device = DeviceTensor::from_host(Arc::clone(backend), b_out.clone()).unwrap();
