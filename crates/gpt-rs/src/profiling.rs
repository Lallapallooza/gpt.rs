...
mod tests {
    ...
    #[test]
    fn nested_scopes_accumulate_exclusive_time_per_key() {
        ...
        {
            let _outer = layer_scope(label);
            sleep_for(outer_before);
            {
                let _inner = layer_scope(label);
                sleep_for(inner_duration);
            }
            sleep_for(outer_after);
        }

        let mut stats = profiler.take_stats();
        let key = ProfilerKey::Layer {
            section: 0,
            name: label,
        };
        let stat = stats
            .get(&key)
            .cloned()
            .expect("expected layer stat for nested guard");
        ...
        assert!(
            stat.exclusive_ns >= expected_exclusive_ns.saturating_sub(tolerance_ns),
            "expected exclusive_ns >= {}ns, got {}",
            expected_exclusive_ns.saturating_sub(tolerance_ns),
            stat.exclusive_ns
        );
        assert!(
            stat.exclusive_ns <= expected_exclusive_ns.saturating_add(tolerance_ns),
            "expected exclusive_ns <= {}ns, got {}",
        ...
        assert!(
            stat.inclusive_ns >= expected_inclusive_ns.saturating_sub(tolerance_ns),
            "expected inclusive_ns >= {}ns, got {}",
            expected_inclusive_ns.saturating_sub(tolerance_ns),
            stat.inclusive_ns
        );
        assert!(
            stat.inclusive_ns <= expected_inclusive_ns.saturating_add(tolerance_ns),
            "expected inclusive_ns <= {}ns, got {}",
        ...
        let summary =
            summarise_tables(std::mem::take(&mut stats)).expect("expected profiler tables");
        let layer_row = &summary.layers[0];
